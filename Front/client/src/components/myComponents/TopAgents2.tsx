...
import { toast } from "sonner"; // ✅ Para exibir a notificação
...
export default function TopAgents() {
  const { users, currentUser } = useUsers(); // ✅ currentUser incluído
  ...
  const handleCardClick = (agentId: string) => {
    if (!currentUser) {
      toast.warning("Você precisa estar logado para ver detalhes do agente.");
      router.push("/login");
      return;
    }

    router.push(`/user/agentes/${agentId}`);
  };
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-6 w-full">
          {topAgents.map((agent) => (
            <Card
              key={agent.id}
              onClick={() => handleCardClick(agent.id!)} // ✅ Chamada controlada
